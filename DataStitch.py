...
class SEM_PFD_Final(torch.utils.data.Dataset):
    def __init__(self, root, transforms=None):
        ...
        self.MasksPath = "PFD_Fin_Masks"
        self.boxPath = "PFD_Fin_boxes"
    ...
    def __getitem__(self, idx):
        ...
        mask_path = os.path.join(self.root, self.MasksPath, "Masks" + str(idx)+ '.csv')
        box_path = os.path.join(self.root, self.boxPath, "boxes" + str(idx)+ '.csv')
...
if __name__ == "__main__":
    ...
    PATHFull = 'C:/Users/sammy/Downloads/SEM/SEM/MaskRCNNTrySherFullModel_3part3.pt'
    ...
    trueImg = np.zeros((l*parts, w*parts))
    cutVal = .5
    for i in range(0, len(dataset)):
        ...
        SubMasks = info['masks']
        SubMask = torch.sum(SubMasks, axis = 0)
        trueMask[a*l:(a+1)*l, b*w:(b+1)*w] = SubMask
        trueImg[a*l:(a+1)*l, b*w:(b+1)*w] = img[0]
plt.imshow(predictMask)
plt.savefig('pm.png')
plt.imshow(trueMask)
plt.savefig('tm.png')
plt.imshow(predictMask - trueMask)
plt.savefig('diff.png')
plt.imshow(trueImg)
plt.savefig('ti.png')
fname = "C:/Users/sammy/Downloads/SEM/SEM/PredictMaskSher3pt3.csv"
np.savetxt(fname, predictMask, fmt = '%d', delimiter=',')
fname = "C:/Users/sammy/Downloads/SEM/SEM/RealMask3pt2.csv"
np.savetxt(fname, trueMask, fmt = '%d', delimiter=',')